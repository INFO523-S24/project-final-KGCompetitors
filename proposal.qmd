...
Our team is participating in a Kaggle Competition with the higher end goal and objective being to create a model capable of detecting personally identifiable information (PII) within student writing. Under our GitHub repository, there is a file titled "analysis", which contains a jupyter notebook of data and task description alongwith a DeBERTa model run for our first attempt at creating a model to detect PII in student writing. We selected DeBERTa as the first model, a transformer based neural language model that has two distinct features.The first is it's disentangled attention mechanism which involves representing each word with two vectors, one encoding its content and the other its position, and computing attention weights between words using disentangled matrices based on their content and relative positions. The second is it's improved mask decoder, which replaces the output softmax layer to predict masked tokens during model pre-training.

We will implement a robust data processing pipeline and model training approach. We'll tokenize text and convert it into integers for model compatibility, adding special tokens like [CLS], [SEP], and [PAD] for enhanced input handling. Leveraging tf.data.Dataset, we'll create separate data loaders for training and validation. The model will be trained using CrossEntropy loss, evaluated using the Fβ-score metric, and will utilize a Dense layer with softmax activation for prediction. Training will be conditionally executed based on the `CFG.train` flag, with progress monitored through epoch updates. Test data will undergo preprocessing for uniform sample sizes and tokenization, with token IDs generated for inference. Post-processing of predictions will filter unwanted parts, ensuring desired output criteria are met.
...
| Analysis | In progress | Monica Tejaswi, Remi Hendershott, G Sai Laasya, Shashank Yadav | 08 Apr 2024 | Moderate|  Team consolidates findings to generate comprehensive questions aimed at exploring deeper insights |
...
| Proposal Instructor Review | In Progress | Everyone | 08 Apr 2024 | Moderate | Attain feedback on proposal from Instructor.|
| Code Peer Review | Upcoming | Everyone | 01 May 2024 | Moderate | Reviewing Other Teams' Code in class together. |
| Write-Up | In progress | Everyone | 06 May 2024 | High | Completing a detailed written document explaining our project, our model, data visualizations of our code chunks and results, as well as our conclusion and findings. |
| Presentation | In progress | Everyone | 06 May 2024 | High | Creating a short, but informative slideshow to present our project and model results to the class. |
...
- **\_analysis/:** Contains the jupyter notebook of our analysis plan of the project.
